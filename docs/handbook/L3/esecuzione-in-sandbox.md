...
uscita verso internet), ma collegabile a caldo (`sandbox.py · L306–311`,
`sandbox.py · L124`).
...
comando. Il docstring lo dice esplicitamente (`sandbox.py · L300–305`).
...
| Byte NUL nel comando | `ValueError` — rifiutato prima di partire (`sandbox.py · L434`) |
...
(`sandbox.py · L446–449`, `sandbox.py · L474–485`).
...
pagando pochi token (`sandbox.py · L489–498`).
...
filesystem non deve far perdere del tutto l'output (`sandbox.py · L516–519`).
...
| Operazione | Evidenza |
|---|---|
| Avvio pigro al primo uso | `sandbox.py · L359–406` (`ensure_running`) |
| Verifica post-avvio | `sandbox.py · L403–404` — se non risulta running, errore esplicito |
| Stop e rimozione rete di sessione | `sandbox.py · L408–419` |
...
**Evidenza:** `sandbox.py · L556–616`.

Tre condizioni prima di fermare, tutte necessarie: il container è tra quelli in esecuzione,
la sessione **non è occupata**, e l'inattività supera la soglia (`sandbox.py · L586–595`).
...
| `src/agent_harness/sandbox.py` | L227–246 | connect/disconnect rete |
| `src/agent_harness/sandbox.py` | L293–356 | flag di confinamento |
| `src/agent_harness/sandbox.py` | L359–419 | avvio pigro, stop, pulizia rete |
| `src/agent_harness/sandbox.py` | L421–472 | esecuzione, concessione rete, eccezioni |
| `src/agent_harness/sandbox.py` | L488–519 | offload dell'output lungo |
| `src/agent_harness/sandbox.py` | L556–616 | reaper di inattività |
| `src/agent_harness/sandbox.py` | L668–681 | esposizione come tool |
...
- **Non sostituire la rete di sessione con `--network none`.** Rompe la concessione
  temporanea di internet. Il motivo è a `sandbox.py · L306–311`.
...
- Il `finally` che revoca la rete a `sandbox.py · L456–458` è un'invariante di sicurezza:
